...
namespace WialonServer.Services
{
    ...
    {
        private WailonDataModel wailonDataModel { get; set; }
        ...
        public WialonParsingService()
        {
            wailonDataModel = new WailonDataModel();
        }
        ...
        public void ParseData()
        {
            ...
            startIndex += 4;
            wailonDataModel.PacketLength = (int)ConvertByteArrayToValue(bufferArray);
           ...
           wailonDataModel.ControllerId = ConvertByteArrayToValue(byteBuffer.ToArray());



            Array.Copy(baseArray, startIndex, bufferArray, 0, 4);
            wailonDataModel.CurrentTime = (int)ConvertByteArrayToValue(bufferArray);
            startIndex += 4;


            Array.Copy(baseArray, startIndex, bufferArray, 0, 4);
            wailonDataModel.Flags = (int)ConvertByteArrayToValue(bufferArray);
            startIndex += 4;
        ...
        private long ConvertByteArrayToValue(byte[] bufferArray)
        {
            int bytePlace = bufferArray.Length - 1;
            long result = 0;
            for (int i = 0; i < bufferArray.Length; i++, bytePlace--)
            {
                result =result + (long)(bufferArray[i] * Math.Pow(256, bytePlace));
            }
            return result;
        }
    }
